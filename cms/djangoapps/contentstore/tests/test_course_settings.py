...
class CourseGradingTest(CourseTestCase):
    ...
    def test_delete_cutoffs(self):
        test_grader = CourseGradingModel.fetch(self.course.location)
        default_cutoffs = test_grader.grade_cutoffs.copy()
        CourseGradingModel.update_cutoffs_from_json(test_grader.course_location, test_grader.grade_cutoffs)
        # Unlike other tests, need to actually perform a db fetch for this test since update_cutoffs_from_json
        #  simply returns the cutoffs you send into it, rather than returning the db contents.
        altered_grader = CourseGradingModel.fetch(self.course.location)
        ...
        for grade, cutoff in (('A', .9), ('B', .75), ('C', .55), ('Pass', .55)):
            test_grader.grade_cutoffs[grade] = cutoff
        CourseGradingModel.update_cutoffs_from_json(test_grader.course_location, test_grader.grade_cutoffs)
        altered_grader = CourseGradingModel.fetch(self.course.location)
        self.assertDictEqual(test_grader.grade_cutoffs, altered_grader.grade_cutoffs, "Change cutoffs")

        for grade, cutoff in (('A', .95), ('B', .85), ('C', .65), ('Pass', .65)):
            test_grader.grade_cutoffs[grade] = cutoff
        # Now "delete" cutoffs (this is a misnomer; should just set grade cutoffs back to the defaults)
        CourseGradingModel.delete_cutoffs(test_grader.course_location)
        # Unlike other tests, need to actually perform a db fetch for this test since delete_cutoffs
        #  simply returns the default cutoffs, rather than returning the db contents.
        altered_grader = CourseGradingModel.fetch(self.course.location)
        self.assertDictEqual(default_cutoffs, altered_grader.grade_cutoffs, "Delete cutoffs")
